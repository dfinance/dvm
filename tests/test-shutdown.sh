...
# check the exit code:
if (( $EXECUTABLE_EXIT_CODE == $EXPECTED_EXIT_CODE || $EXECUTABLE_EXIT_CODE == $EXPECTED_EXIT_CODE_ALT )); then
  echo "exit-code: OK"
else
  echo "exit-code: ERR: $EXECUTABLE_EXIT_CODE != $EXPECTED_EXIT_CODE or $EXPECTED_EXIT_CODE_ALT"
  exit 1
fi
